...
// type PendingSignal = { type: 'offer' | 'answer' | 'iceCandidate'; data: any };
...
//       sock.on('reconnect', () => {
//         console.log('Reconnected to Socket.IO server. Rejoining video call...');
...
//           console.log('Local media stream obtained and set.');
//         }
...
//     return () => {
//       console.log('Performing cleanup on component unmount (DoctorVideoCall)...');
//       if (currentSocket) {
...
//           console.log(`Stopped track: ${track.kind}`);
//         });
//         console.log('Local stream tracks stopped on unmount.');
//       }
//       if (peerConnection.current) {
//         peerConnection.current.close();
//         peerConnection.current = null;
...
//     };
//   }, [appointmentId, navigate, getAccessToken]);

//   useEffect(() => {
//     let timeout: ReturnType<typeof setTimeout>;
//     if (!otherUserJoined && callStatus === 'waiting') {
//       console.log('Setting 5-minute timeout for patient to join...');
//       timeout = setTimeout(() => {
//         if (!otherUserJoined) {
//           message.warning('Patient has not joined the call within 5 minutes. Returning to appointments...');
//           endCall();
//         }
//       }, 5 * 60 * 1000);
//     }
...
//       console.log(`Socket is ready. Emitting 'joinVideoCall' for appointment: ${appointmentId}`);
//       socket.emit('joinVideoCall', appointmentId);

//       socket.on('joinedVideoCall', (data) => {
//         console.log('Successfully joined video call room on server:', data.appointmentId);
//       });

//       socket.on('userJoined', (data: { userId: string; role: 'doctor' | 'user' }) => {
//         console.log(`User ${data.userId} (${data.role}) joined the call room. Setting otherUserJoined to true.`);
//         setOtherUserJoined(true);
//         setCallStatus('connected');
//         if (localStream && !peerConnection.current) {
//           console.log("Local stream ready and peer connection not initialized. Calling initializePeerConnection.");
//           initializePeerConnection();
//         } else if (!localStream) {
//           console.warn("Local stream not yet available when userJoined. PeerConnection might be delayed.");
//         }
//       });

//       socket.on('startCall', () => {
//         console.log('Received startCall event from server. Attempting to create WebRTC offer...');
//         if (!peerConnection.current) {
...
//       return () => {
//         console.log('Removing Socket listeners for DoctorVideoCall.');
...
//       };
//     }
//   }, [socket, appointmentId, localStream, createOffer, handleOffer, handleAnswer, handleIceCandidate, endCall, initializePeerConnection]);

//   return (
//     <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-100 flex items-center justify-center p-4">
//       <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-4xl">
//         <h1 className="text-3xl font-bold text-gray-800 mb-6 text-center">Video Consultation with Patient</h1>
//         {error && (
...
//             />
//             <p className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-black bg-opacity-50 text-white px-3 py-1 rounded-full text-sm">You (Doctor)</p>
//           </div>
//           <div className="flex-1 relative bg-gray-900 rounded-lg overflow-hidden">
...
//               </div>
//             )}
//             <p className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-black bg-opacity-50 text-white px-3 py-1 rounded-full text-sm">Patient</p>
//           </div>
//         </div>
...
export default function DoctorVideoCall() {
  ...
  const handleCallUser = useCallback(async () => {
    if (!localStream || !socketRef.current || !remoteSocketId) {
      console.error("Cannot initiate call: localStream, socket, or remoteSocketId not ready");
      return;
    }
    try {
      ...
      if (offer) {
        ...
        console.log("Sent call offer to", remoteSocketId);
        sendStreams();
      }
    } catch (err) {
  ...
  }, [localStream, remoteSocketId, sendStreams]);
    ...
    const setup = async () => {
      ...
      try {
        ...
        socket.on("joinedVideoCall", () => {
          console.log("Doctor joined video call:", appointmentId);
          setCallStatus("waiting");
        });
        ...
        socket.on("startCall", () => {
          console.log("Received startCall event for doctor");
          handleCallUser();
        });
        ...
        socket.on("mute", ({ type, muted, userId }) => {
          message.info(`Patient ${muted ? "muted" : "unmuted"} ${type}.`);
        });
        ...
        socket.on("callEnded", ({ userId }) => {
          ...
          cleanupCall();
          setCallStatus("ended");
          message.info("Call ended by patient.");
        ...
        socket.on("userLeft", ({ userId }) => {
          ...
          message.info("Patient has left the call.");
        });
  ...
  }, [appointmentId, getToken, navigate, handleCallUser, sendStreams]);
