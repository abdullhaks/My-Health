...
const DoctorSlots = () => {
  ...
  useEffect(() => {
    const validateSessions = () => {
      ...
      sessions.forEach((session, index) => {
        ...
        if (start >= end) {
          errors.push({ index, message: "End time must be after start time." });
          return;
        }
        ...
        sameDaySessions.forEach((otherSession) => {
          ...
          if (
            ...
            errors.push({
              index,
              message: `This session overlaps with another session on ${
                weekdays.find((day) => day.value === session.dayOfWeek)?.name
              }.`,
            });
          }
        });

        // Validate rRule matches dayOfWeek if rRule exists
        if (session.rRule) {
          try {
            const rrule = rrulestr(session.rRule);
            const rruleDay = rrule.options.byweekday[0];
            if (rruleDay !== session.dayOfWeek) {
              errors.push({
                index,
                message: `Recurrence rule day does not match selected day (${
                  weekdays.find((day) => day.value === session.dayOfWeek)?.name
                }).`,
              });
            }
          } catch (error) {
            errors.push({ index, message: "Invalid recurrence rule." });
          }
        }
      });
  ...
  useEffect(() => {
    ...
    const generateSlotsForDate = (date: Date) => {
      ...
      daySessions.forEach((session) => {
        ...
        //   try {
        //     const rrule = rrulestr(session.rRule);
        //     // Convert date to UTC for rRule comparison
        //     const utcDate = new Date(
        //       Date.UTC(date.getFullYear(), date.getMonth(), date.getDate())
        //     );
        //     shouldGenerateSlots = rrule.between(utcDate, utcDate, true).length > 0;
        //     console.log(`rRule check for session ${session.id}:`, shouldGenerateSlots);
        //   } catch (error) {
        //     console.error(`Invalid rRule for session ${session.id}:`, error);
  ...
  //     freq: RRule.WEEKLY,
  //     byweekday: [weekdayMap[session.dayOfWeek]],
  ...
  const saveSessions = async () => {
    ...
    try {
      ...
      const sessionsWithRRule = sessions.map((session) => {
        // const rRule = generateRRule(session);
        // console.log(`Generated rRule for session ${session.id || "new"}:`, rRule);
        return { ...session,doctorId: doctor._id };
      });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg p-6 shadow">
          ...
          <Calendar
            ...
            value={selectedDate}
            tileClassName={({ date }) =>
              sessions.some((s) => s.dayOfWeek === date.getDay()) ? "bg-blue-50" : ""
            }
          />
          <div className="mt-6">
